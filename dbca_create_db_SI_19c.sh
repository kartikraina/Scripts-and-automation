...
DB_UNQ_NAME=devsalep
DB_TYPE=MULTIPURPOSE
DB_NAME=devsalep
DB_SID_NAME=devsalep
DB_HOME=/u01/app/oracle/product/19.3.0.0/dbee_1
ORA_BASE=/u01/app/oracle
DB_SYS_PASSWORD="Y<]ApDHP6Ur6[k"
DB_SYSTEM_PASSWORD="xqu2R]$t9sVNqY"
DB_STORAGE_TYPE=ASM
DATAFILE_DEST=+DEVSALEP_DG
ARCH_DEST=+ORASHA2S_ARCH_DG
ASMSNMP_PASSWORD="Y<]ApDHP6Ur6[k"
DB_CHARSET=AL32UTF8
DB_NCHARSET=AL16UTF16
ENABLE_ARCHIVE=true
ONLINE_LOG_FILE_SIZE_IN_MB=200
...
#datafileDestination=+STGSVEP_DG/{DB_UNIQUE_NAME}/
storageType=$DB_STORAGE_TYPE
#diskGroupName=+STGSVEP_DG/{DB_UNIQUE_NAME}/
...
initParams=undo_tablespace=$UNDO_TB,sga_target=$SGA_TGT,db_files=$MAX_DB_FILES,db_block_size=${DB_BLK_SIZE_IN_BYTES}BYTES,cluster_database=false,,nls_language=$NLS_LANG,dispatchers=(PROTOCOL=TCP)(SERVICE=${DB_UNQ_NAME}XDB),diagnostic_dest=$DIAGNOSTIC_DEST,remote_login_passwordfile=exclusive,db_create_file_dest=$DATAFILE_DEST,audit_file_dest=$AUDIT_DEST,processes=$PROCESSES,pga_aggregate_target=$TGT,nls_territory=$NLS_TERRITORY,log_archive_format=%t_%s_%r.dbf,compatible=$COMPATIBLE,db_name=$DB_NAME,audit_trail=db,db_create_online_log_dest_1=$DATAFILE_DEST,db_create_online_log_dest_2=$ARCH_DEST,db_recovery_file_dest_size=50G,db_recovery_file_dest=$ARCH_DEST,log_archive_dest_1='LOCATION=${ARCH_DEST}'
sampleSchema=false
memoryPercentage=40
...
$DB_HOME/bin/dbca -createDatabase -responseFile ${RESPONSE_FILE} -enableArchive $ENABLE_ARCHIVE -redoLogFileSize $ONLINE_LOG_FILE_SIZE_IN_MB >> $INSTALL_LOG_LOCATION/${DB_UNQ_NAME}_dbca.log 2>&1
