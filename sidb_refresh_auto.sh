#script to refresh to SIDB from production using active duplication
#  usage : nohup /u01/app/product/oracle/admin/db_auto_refresh.sh esors1 esors1 >> /u01/app/product/oracle/admin/esors1/audit/esors1_db_refresh_$(date +%d_%m_%Y_%H%M%S).log 2>&1 &
#
###############################################
#set -x
#clear
echo "*************************************************************************"
echo "====>Script sidb_refresh_auto.sh Starting on" `date`
echo "*************************************************************************"
...
{
echo "$ORACLE_HOME/bin/orapwd file=$ORACLE_HOME/dbs/orapw${ORACLE_SID} password=********* force=y"

$ORACLE_HOME/bin/orapwd file='$ORACLE_HOME/dbs/orapw${ORACLE_SID}' password=\"$SRC_DB_PSWD\" force=y
}
...
echo
    ...
    --create spfile='$ORACLE_HOME/dbs/spfile$ORACLE_SID.ora' from pfile='$ORACLE_HOME/dbs/init$ORACLE_SID.ora.refresh.$$';
    shut immediate
    exit
EOF

#echo "====>Shutting down and Starting database on all available RAC nodes"
echo "====>Shutting down and Starting database using spfile."
$ORACLE_HOME/bin/sqlplus /nolog <<EOF >${audit_path}/ora.error.$$
connect / as sysdba
startup mount;
alter database open;
...
#srvctl status database -d $DB_UNIQUE_SID -v
$ORACLE_HOME/bin/sqlplus /nolog << EOF >> ${audit_path}/ora.error.$$
connect / as sysdba
select name,open_mode,controlfile_type,database_role from v\$database;
exit
...
#srvctl start database -d $DB_UNIQUE_SID
...
clear
echo "*************************************************************************"
echo "====>Script sidb_refresh_auto.sh Ending on" `date`
echo "*************************************************************************"
